    ...
    if(axisnum<PULS_NUM)	//只有主卡部分点动函数
    {
        if(pos > 0)	//正向点动
        {
            if(AXSTA_ERRSTOP == HZ_AxGetStatus(axisnum))
            {
                ...
                {
                    HZ_AxReset(axisnum);
					if(axisnum==MLMOTOR/*&&InGet(I_Moveline_Lim)==OFF*/&&InGet(I_MlArm_Up)==ON) //判断安全)
					{	
                ...
                //没有报警,正常运动
				if(axisnum==MLMOTOR&&InGet(I_MlArm_Up)==ON) //判断安全)
				{	
					HZ_AxMoveRel(axisnum,pos);
				}
				else 
        ...
        else	//反向点动
        {
            if(AXSTA_ERRSTOP == HZ_AxGetStatus(axisnum))
            {
                ...
                {
                    HZ_AxReset(axisnum);
					if(axisnum==0&&InGet(I_MlArm_Up)==ON)
                    {
						HZ_AxMoveRel(axisnum,pos);
					}else
					{
                ...
                //没有报警,正常运动
				if(axisnum==0&&InGet(I_MlArm_Up)==ON) //判断安全)
				{	
					HZ_AxMoveRel(axisnum,pos);
				}
				else 
